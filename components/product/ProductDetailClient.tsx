...
export default function ProductDetailClient({
  ...
  const [clickedPreview, setClickedPreview] = useState<PreviewData | null>(
    null
  );
    ...
    <div>
      <div className="md:grid md:grid-cols-[40%_auto] md:items-start md:gap-x-12 py-8">
        ...
        <div>
          ...
          {!isMobile && product.related_blogs && (
            <RelatedArticles blogs={product.related_blogs} lang={lang} />
          )}

          {!isMobile && product.related_products && product.related_products.length > 0 && (
            <Cav products={product.related_products} lang={lang} />
          )}
        </div>
